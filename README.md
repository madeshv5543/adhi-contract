...
Better Adhichain contract abstraction, for Node and the browser.
...
With this usage, `adhi-contract` will be available via the `TruffleContract` object:
...
Let's use `adhi-contract` with an example contract from [Dapps For Beginners](https://dappsforbeginners.wordpress.com/tutorials/your-first-dapp/). In this case, the abstraction has been saved to a `.sol.js` file by [truffle-artifactor](https://github.com/trufflesuite/truffle-artifactor):
